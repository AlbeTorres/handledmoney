...
vi.mock('react', async importOriginal => {
  ...
  return { ...actual, useTransition: () => [navigation.pending, (callback: () => void) => callback()] }
})
...
import {
  ...
  nextPeriod,
  previousPeriod,
} from '@/app/(financeapp)/dashboard/_components/dashboard-period-header'

const now = new Date(Date.UTC(2026, 5, 15))
const dashboardData = {
  period: { mode: 'monthly' as const, year: 2026, month: 5 },
  kpis: { incomePlanned: 0, incomeActual: 0, expensePlanned: 0, expenseActual: 0, netPlanned: 0, netActual: 0, available: 0 },
  groups: [],
  budgetVsActual: [],
  monthlyTrend: [],
  expenseByGroup: [],
...
describe('dashboard URL period selection', () => {
  ...
  it('parses only canonical scalar monthly and annual periods', () => {
    expect(parseDashboardPeriod({ mode: 'monthly', year: '2026', month: '11' }, now)).toEqual({
      mode: 'monthly', year: 2026, month: 11,
    })
    expect(parseDashboardPeriod({ mode: 'annual', year: '2026' }, now)).toEqual({ mode: 'annual', year: 2026 })
  })
  ...
  it('renders the selected server period in the dashboard header', async () => {
    ...
    render(await Dashboard({ searchParams: Promise.resolve({ mode: 'monthly', year: '2026', month: '5' }) }))
  ...
  it('renders a non-sensitive retryable alert without dashboard controls when data is unavailable', async () => {
    ...
    expect(screen.queryByRole('heading', { name: 'Presupuesto vs Realidad' })).not.toBeInTheDocument()
...
describe('DashboardPeriodHeader', () => {
  it('renders the exact Spanish copy, selected mode, and balance states', () => {
    const { rerender } = render(<DashboardPeriodHeader period={{ mode: 'monthly', year: 2026, month: 0 }} netActual={12} />)

    expect(screen.getByRole('heading', { name: 'Presupuesto vs Realidad' })).toBeInTheDocument()
    expect(screen.getByText('Compara tus flujos estimados contra tu desempeño real.')).toBeInTheDocument()
  ...
  it('builds pathname-prefixed canonical URLs and crosses period boundaries', () => {
    expect(hrefForPeriod('/dashboard', { mode: 'monthly', year: 2026, month: 11 })).toBe('/dashboard?mode=monthly&year=2026&month=11')
    expect(hrefForPeriod('/dashboard', { mode: 'annual', year: 2026 })).toBe('/dashboard?mode=annual&year=2026')
    expect(nextPeriod({ mode: 'monthly', year: 2026, month: 11 })).toEqual({ mode: 'monthly', year: 2027, month: 0 })
    expect(previousPeriod({ mode: 'monthly', year: 2027, month: 0 })).toEqual({ mode: 'monthly', year: 2026, month: 11 })
  ...
  it('pushes canonical URLs from mouse and keyboard controls and preserves the year on mode changes', async () => {
    ...
    const { rerender } = render(<DashboardPeriodHeader period={{ mode: 'monthly', year: 2026, month: 11 }} netActual={0} />)
    ...
    expect(navigation.push).toHaveBeenLastCalledWith(`/dashboard?mode=monthly&year=2026&month=${new Date().getUTCMonth()}`)
  })

  it('keeps controls accessible while pending and announces only matching refreshed server props', async () => {
    const user = userEvent.setup()
    const { rerender } = render(<DashboardPeriodHeader period={{ mode: 'annual', year: 2026 }} netActual={0} />)
