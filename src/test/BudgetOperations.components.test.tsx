...
const mocks = vi.hoisted(() => ({
  createGroup: vi.fn(), updateGroup: vi.fn(), deleteGroup: vi.fn(), createItem: vi.fn(), getCategories: vi.fn(), createCategory: vi.fn(),
  setCurrent: vi.fn(), duplicate: vi.fn(), updateBudget: vi.fn(), deleteBudget: vi.fn(), push: vi.fn(),
  success: vi.fn(), error: vi.fn(),
}))
vi.mock('@/actions/budget/budget-item', () => ({ createBudgetGroupAction: mocks.createGroup, updateBudgetGroupAction: mocks.updateGroup, deleteBudgetGroupAction: mocks.deleteGroup, createBudgetItemAction: mocks.createItem }))
vi.mock('@/actions/category/get-categories', () => ({ getCategoriesByUserAction: mocks.getCategories }))
vi.mock('@/actions/category/create-category', () => ({ createCategoryAction: mocks.createCategory }))
vi.mock('@/actions/budget/select-current-budget', () => ({ selectCurrentBudgetAction: mocks.setCurrent }))
...
describe('Budget operation components', () => {
  beforeEach(() => {
    ...
    mocks.createCategory.mockResolvedValue({ success: true, data: { id: 'category-1', name: 'Salary' } })
  ...
  it('creates an outflow group with the budget-owned sort order', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /^add group$/i }))
    await waitFor(() => expect(mocks.createGroup).toHaveBeenCalledWith({ budgetId, name: 'Home', calculationType: 'outflow', sortOrder: 6 }, budgetId))
  })

  it('renames a rendered budget group', async () => {
    const user = userEvent.setup()
    render(<BudgetGroupSection budgetId={budgetId} group={{ id: 'group-1', budgetId, name: 'Bills', calculationType: 'outflow', sortOrder: 1, createdAt: new Date(), updatedAt: new Date(), items: [], groupPlanned: 0, groupActual: 0 }} />)
    ...
    await waitFor(() => expect(mocks.updateGroup).toHaveBeenCalledWith({ id: 'group-1', name: 'Home' }, budgetId))
  })

  it('filters item categories by group type and auto-selects an inline created category', async () => {
    const user = userEvent.setup()
    render(<AddBudgetItemForm budgetId={budgetId} groupId='00000000-0000-0000-0000-000000000002' calculationType='income' />)
    ...
    await waitFor(() => expect(mocks.createCategory).toHaveBeenCalledWith(expect.objectContaining({ name: 'Salary', type: 'income' })))
  ...
  it('marks the current card and exposes current selection plus dated duplication controls', async () => {
    const user = userEvent.setup()
    render(<><BudgetGrid budgets={[budget]} currentBudgetId={budgetId} /><BudgetDetailView budget={budget} /></>)
    ...
    await waitFor(() => expect(mocks.duplicate).toHaveBeenCalledWith(expect.objectContaining({ id: budgetId, startDate: expect.any(Date) })))
  })
