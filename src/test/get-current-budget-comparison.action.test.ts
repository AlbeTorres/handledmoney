...
import { getCurrentBudgetComparisonAction } from '@/actions/budget/get-current-budget-comparison'
...
describe('getCurrentBudgetComparisonAction', () => {
  ...
  it('returns only the authenticated user comparison', async () => {
    ...
    await expect(getCurrentBudgetComparisonAction()).resolves.toMatchObject({ success: true, data: { budget: { id: 'budget-1' } } })
    expect(mocks.getComparison).toHaveBeenCalledWith('user-1')
  })

  it('rejects unauthenticated requests', async () => {
    mocks.getSession.mockResolvedValue(null)
    await expect(getCurrentBudgetComparisonAction()).resolves.toMatchObject({ success: false, status: 401 })
