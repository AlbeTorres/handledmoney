...
const mocks = vi.hoisted(() => ({ getSession: vi.fn(), getCurrentBudget: vi.fn(), headers: vi.fn() }))
...
describe('getCurrentBudgetAction', () => {
  ...
  it('returns the authenticated user current selection', async () => {
    ...
    await expect(getCurrentBudgetAction()).resolves.toMatchObject({ success: true, data: { budgetId: 'budget-1' } })
