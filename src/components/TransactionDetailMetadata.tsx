    ...
    <section className='rounded-sm border bg-white p-5 shadow-sm dark:bg-slate-900'>
      <dl className='space-y-3'>
        <div className='flex items-start justify-between gap-4'>
          <dt className='text-sm font-medium text-muted-foreground'>{t('transaction_id')}</dt>
          <dd className='font-mono text-sm text-foreground'>
            <p className='wrap-break-word w-full break-all whitespace-pre-wrap text-right'>{id}</p>
          </dd>
        </div>
        <div className='flex items-start justify-between gap-4'>
          <dt className='text-sm font-medium text-muted-foreground'>{t('added_on')}</dt>
          <dd className='text-sm text-foreground'>{fmtDate(createdAt)}</dd>
        </div>
      </dl>
    </section>
  )
