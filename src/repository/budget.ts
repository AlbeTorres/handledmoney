    ...
    for (const group of groups) {
      for (const item of group.items) {
        ...
        if (category.type !== expectedType)
          throw new Error('Category type does not match group type')
      }
    }

    const [budget] = await tx
      .insert(budgetsTable)
      .values({ userId, name, startDate, endDate: endDate ?? null })
      .returning()
    const createdGroups = await Promise.all(
      groups.map(group =>
        tx
          .insert(budgetGroupsTable)
          .values({
            budgetId: budget.id,
            name: group.name,
  ...
  const groups: BudgetGroupWithItems[] = budget.groups.map(group => {
    ...
    const items: BudgetItemWithActual[] = group.items.map(item => {
      ...
      return {
        ...
        usageRate: plannedAmount ? actualAmount / plannedAmount : Number.NaN,
      }
    })
...
export const updateBudget = async ({ id, ...values }: UpdateBudgetValues, userId: string) => {
  const current = await db.query.budgetsTable.findFirst({
    where: and(eq(budgetsTable.id, id), eq(budgetsTable.userId, userId)),
  })
...
export const selectCurrentBudget = async (budgetId: string, userId: string) => {
  ...
  const [selection] = await db
    .insert(currentBudgetsTable)
    .values({ userId, budgetId })
    .onConflictDoUpdate({ target: currentBudgetsTable.userId, set: { budgetId } })
    .returning()
  return selection
}
 ...
 */
export const getCurrentBudgetComparison = async (
  userId: string,
): Promise<CurrentBudgetComparison | null> => {
  ...
  const actualByCategory = new Map<
    string,
    { categoryName: string; type: 'income' | 'expense'; actualAmount: number }
  >()
  ...
  for (const planned of plannedByCategory.values()) {
    ...
    const comparison = {
      ...planned,
      actualAmount: actual,
      variance: planned.plannedAmount - actual,
    }
...
export const duplicateBudget = async (
  { id, name, startDate }: DuplicateBudgetValues,
  userId: string,
) => {
...
export const createBudgetGroup = async (values: CreateBudgetGroupValues, userId: string) => {
  const budget = await db.query.budgetsTable.findFirst({
    where: and(eq(budgetsTable.id, values.budgetId), eq(budgetsTable.userId, userId)),
  })
  if (!budget) return undefined
...
export const updateBudgetGroup = async (
  { id, name }: UpdateBudgetGroupValues,
  budgetId: string,
  userId: string,
) => {
  const budget = await db.query.budgetsTable.findFirst({
  ...
  if (!budget) return undefined
  const [updated] = await db
    .update(budgetGroupsTable)
    .set({ name })
    .where(and(eq(budgetGroupsTable.id, id), eq(budgetGroupsTable.budgetId, budgetId)))
    .returning()
  return updated
}

export const deleteBudgetGroup = async (id: string, budgetId: string, userId: string) => {
  const group = await db.query.budgetGroupsTable.findFirst({
    where: and(eq(budgetGroupsTable.id, id), eq(budgetGroupsTable.budgetId, budgetId)),
  })
  ...
  if (group.calculationType === 'income') {
    const [incomeGroups] = await db
      .select({ total: count() })
      .from(budgetGroupsTable)
      .where(
        and(
          eq(budgetGroupsTable.budgetId, budgetId),
          eq(budgetGroupsTable.calculationType, 'income'),
        ),
      )
    if (Number(incomeGroups.total) <= 1)
      throw new Error('A budget must include at least one income group')
  }
  const [deleted] = await db
...
export const createBudgetItem = async (values: CreateBudgetItemValues, userId: string) => {
  const group = await db.query.budgetGroupsTable.findFirst({
    where: eq(budgetGroupsTable.id, values.groupId),
  })
  if (!group) return undefined
  const budget = await db.query.budgetsTable.findFirst({
    where: and(eq(budgetsTable.id, group.budgetId), eq(budgetsTable.userId, userId)),
  })
  if (!budget) return undefined
  if (values.categoryId) {
    const category = await db.query.categoriesTable.findFirst({
      where: and(eq(categoriesTable.id, values.categoryId), eq(categoriesTable.userId, userId)),
    })
    if (!category) throw new Error('Category not found')
    if ((group.calculationType === 'income') !== (category.type === 'income'))
      throw new Error('Category type does not match group type')
  }
  const [item] = await db
    .insert(budgetItemsTable)
    .values({ ...values, budgetId: group.budgetId, plannedAmount: String(values.plannedAmount) })
    .returning()
  return item
}

export const updateBudgetItem = async (
  { id, plannedAmount, ...rest }: UpdateBudgetItemValues,
  budgetId: string,
  userId: string,
) => {
  const item = await db.query.budgetItemsTable.findFirst({
    where: and(eq(budgetItemsTable.id, id), eq(budgetItemsTable.budgetId, budgetId)),
  })
  ...
  if (rest.categoryId !== undefined && rest.categoryId !== null) {
    const group = await db.query.budgetGroupsTable.findFirst({
      where: and(eq(budgetGroupsTable.id, item.groupId), eq(budgetGroupsTable.budgetId, budgetId)),
    })
    if (!group) return undefined
    const category = await db.query.categoriesTable.findFirst({
      where: and(eq(categoriesTable.id, rest.categoryId), eq(categoriesTable.userId, userId)),
    })
    if (!category) throw new Error('Category not found')
    if ((group.calculationType === 'income') !== (category.type === 'income'))
      throw new Error('Category type does not match group type')
