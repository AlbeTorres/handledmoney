...
export function KpiCards({ kpis }: Props) {
  ...
  const cards: {
    ...
    },
    {
      ...
      tone: kpis.expenseActual > kpis.expensePlanned ? 'destructive' : 'neutral',
      footer: (
        <Delta
    ...
    },
    {
      ...
      tone: kpis.netActual >= 0 ? 'success' : 'destructive',
      footer: (
    ...
    },
    {
      ...
      tone: kpis.available >= 0 ? 'success' : 'destructive',
      footer: (
        <span className='text-xs text-muted-foreground'>
          {kpis.available >= 0 ? 'Dinero sin asignar' : 'Presupuesto sobreasignado'}
        </span>
      ),
    },
  ]
      ...
      {cards.map(c => (
        ...
        >
          <div className='flex items-center justify-between'>
            <span className='text-sm font-medium text-muted-foreground text-pretty'>{c.label}</span>
            <span
              className={cn(
                'flex size-8 items-center justify-center rounded-lg',
                c.tone === 'success' && 'bg-success/12 text-success',
                c.tone === 'destructive' && 'bg-destructive/12 text-destructive',
