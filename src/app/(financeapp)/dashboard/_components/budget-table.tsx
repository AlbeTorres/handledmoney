...
// Scoped display labels for synthetic groups/categories built in lib/dashboard/budget
const GROUP_LABEL: Partial<Record<GroupRow['id'], string>> = {
  'unplanned-income': 'Ingresos sin presupuestar',
  'unplanned-expense': 'Gastos sin presupuestar',
  'uncategorized-income': 'Ingresos sin categoría',
  'uncategorized-expense': 'Gastos sin categoría',
}

function groupDisplayName(group: GroupRow): string {
  return GROUP_LABEL[group.id] ?? group.name
}

function categoryDisplayName(cat: CategoryRow): string {
  return cat.id.startsWith('uncategorized-') ? 'Sin categoría' : cat.name
}
            ...
            <div key={group.id} className='p-3 sm:p-4'>
              ...
              <div className='flex items-center justify-between rounded-lg bg-muted/60 px-3 py-2'>
                <div className='flex items-center gap-2'>
                  ...
                  />
                  <h3 className='text-sm font-semibold'>{groupDisplayName(group)}</h3>
                  ...
                  <li key={cat.id}>
                    <div className='flex flex-col gap-2 rounded-xl px-2 py-3'>
                      <div className='flex items-center justify-between gap-3'>
                        ...
                        <div className='flex items-center gap-3'>
                          <span className='font-mono text-sm tabular-nums'>
                            <span className='font-semibold'>{formatCurrency(cat.actual)}</span>
                      ...
                      <div className='flex items-center gap-3'>
                        <div
                          role='progressbar'
                          aria-label={`${categoryDisplayName(cat)} — ${Math.round(cat.pct * 100)}% del presupuesto`}
                          aria-valuemin={0}
                          aria-valuemax={100}
...
function DiffBadge({ cat }: { cat: CategoryRow }) {
  ...
  const label =
    cat.kind === 'income' ? (positive ? 'extra' : 'faltante') : positive ? 'disponible' : 'excedido'
