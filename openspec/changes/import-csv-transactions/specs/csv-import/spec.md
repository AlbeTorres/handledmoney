...
Bulk import of real bank and expense-app CSV exports into a selected account: upload, column mapping, normalization, review with per-row flags, and atomic persistence with correct balance effects and a per-row error report. Full spec for the new `csv-import` capability (first main spec at archive; `openspec/specs/` is empty). Covers proposal modules M1–M7 and decisions M0.1–M0.10.
...
Test data for verification, derived from the proposal's edge cases.
